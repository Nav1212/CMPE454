...
void Renderer::render(seq<wfModel *> &objs, mat4 &WCS_to_VCS, mat4 &VCS_to_CCS, vec3 &lightDir, float worldRadius,
                      GLFWwindow *window)

{
    ...
    glClear(GL_COLOR_BUFFER_BIT |
            GL_DEPTH_BUFFER_BIT);  // OpenGL's depth buffer, not the fragment shader's 'depthBuffer' texture!
    ...
    if (firstTime) {
        ...
        for (int i = 0; i < NUM_SAMPLE_OFFSETS; i++) {
            ...
            //       y in [-1, +1]
            //       z in [0,  +1]
            // if (x^2 + y^2 + z^2) >1 or <0.1, reject!

            float x, y, z, len;

            do {
                x = randIn01() * 2.0 - 1.0;  // generate random x in [-1, 1]
                y = randIn01() * 2.0 - 1.0;  // generate random y in [-1, 1]
    ...
    glClear(GL_COLOR_BUFFER_BIT);
    glDisable(GL_DEPTH_TEST);  // not needed as we're drawing the fullscreen quad
    ...
    pass3Prog->setInt("depthBuffer",
                      BASE_GBUFFER_TEXTURE_UNIT + 2);  // CHANGED: ADDED TO ALLOW ACCESS TO DEPTH BUFFER IN pass3.frag
    pass3Prog->setFloat("blurRadius", blurRadius);     // radius of blurring kernel
    pass3Prog->setVec2(
        "texelSize",
        vec2(1.0 / (float)gbuffer->fbWidth,
             1.0 / (float)gbuffer->fbHeight));  // distance between [0,1]x[0,1] texCoords of adjacent texels
